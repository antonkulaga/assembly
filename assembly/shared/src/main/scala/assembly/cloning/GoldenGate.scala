...
case class GoldenGate(enzyme: RestrictionEnzyme) {
  ...
  def synthesize(sequences: Seq[String], flankLeft: String = "", flankRight: String = ""): List[String] = {
    ...
    val updated = (sequences :+ "").tail.sliding(2, 1).map{
      case one::""::Nil => addLeft(one) + flankRight
      case one::two::Nil => addLeft(addRight(one, two))
    }.toList
    val first = flankLeft + addRight(sequences.head, sequences.tail.head)
    first::updated
