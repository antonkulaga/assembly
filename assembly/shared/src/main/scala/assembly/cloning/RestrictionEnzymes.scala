...
/**
  ...
  * @param enzymes2seq
  */
case class RestrictionEnzymes(enzymes2seq: Set[(String, String)]) {
  ...
  def enzymesSites(enzymes: Set[String], reverseComplement: Boolean = true): Set[String] = {
    ...
    if(reverseComplement) sites ++ sites.map(s=>convertStringSequence(s).complement.reverse) else sites
  }

  def notIncludesEnzymes(where: String, enzymes: Set[String], reverseComplement: Boolean = true): Boolean = {
    val sites: Set[String] = enzymesSites(enzymes, reverseComplement)
...
object RestrictionEnzymes {
  ...
  lazy val commonEnzymesSet: Set[(String, String)] = Set(
    "SinI" -> "GGWCC",
    "BamHI" -> "GGATCC",
    "BmeRI" -> "GACNNNNNGTC",
    "SgrAI" -> "CRCCGGYG",
  ...
  lazy val default = RestrictionEnzymes(commonEnzymesSet)
}